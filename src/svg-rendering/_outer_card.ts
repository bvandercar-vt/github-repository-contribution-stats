import { type ColumnName, type ColumnIcon, type ColumnCriteria } from '../common/schema';
...
export function renderCard({
  ...
  colors = {},
  css = '',
  animations = process.env.NODE_ENV !== 'test',
}: {
  customTitle?: string;
  defaultTitle?: string;
  titlePrefixIcon?: string;
  body: string;
  columns: Array<Pick<ColumnCriteria, 'name' | 'icon'> & { width: number }>;
  /**
  ...
  `;
    ...
    const createIcon = (inner: string, { viewBox }: { viewBox: string }) => `
      <svg
        class="icon"
        viewBox="${viewBox}"
      >
    ...
    // from https://fonts.google.com/icons?icon.size=24
    const commitsIcon = createIcon(
    ...
    const rankingsIcon = createIcon(
      `<path d="M160-200h160v-320H160v320Zm240 0h160v-560H400v560Zm240 0h160v-240H640v240ZM80-120v-480h240v-240h320v320h240v400H80Z"/><path xmlns="http://www.w3.org/2000/svg" d="M160-200h160v-320H160v320Zm240 0h160v-560H400v560Zm240 0h160v-240H640v240ZM80-120v-480h240v-240h320v320h240v400H80Z"/>`,
      { viewBox: '0 -960 960 960' },
    );
    ...
    } satisfies Record<ColumnIcon, string>;
    ...
    } satisfies Record<ColumnName, string>;
    ...
    const wrapIcon = (
      icon: string,
      { width, title }: { width: number; title?: string },
    ) => `<svg 
            width="${width}"
            height="24"
            x="0"
